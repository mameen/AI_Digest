...
just because no verifiable link exists. ``strip_ungrounded`` (drop) is retained
...
def collect_skeleton_urls(skeleton: dict[str, Any] | None) -> set[str]:
    ...
    so the guard does not demote a legitimately-sourced curated link. For
    aisearch, URLs embedded in the YouTube video description are included too.
    """
    urls: set[str] = set()
        ...
        for story in cat.get("stories") or []:
            ...
            if isinstance(story, dict):
                for link in story.get("links") or []:
                    ...
                    if link_url:
                        urls.add(normalize_url(str(link_url)))
        desc = cat.get("_video_description") or cat.get("video_description") or ""
        if desc:
            urls |= collect_ingestion_urls(str(desc))
        if cat.get("id") == "youtube" or cat.get("category") == "youtube":
            for src in cat.get("sources") or []:
                src_desc = src.get("description") or ""
                if src_desc:
                    urls |= collect_ingestion_urls(str(src_desc))
                for topic in src.get("topics") or []:
                    if topic.get("url"):
                        urls.add(normalize_url(str(topic["url"])))
    urls.discard("")
    return urls
