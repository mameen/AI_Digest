...
What this script fetches (all in parallel, ~15s):
  ...
  ✓ Typography   — Monotype, I Love Typography, Typographica, MyFonts
...
Output:  .reports/preflight_YYYYMMDDHHMMSS.json
...
def save(data: dict, path: Path | None = None) -> Path:
    out = path or REPORTS_DIR / f"preflight_{data['prefix']}.json"
...
def main():
    ...
    print(f"\n   Tell Claude:\n"
          ...
          f"Read .reports/preflight_{prefix}.json, "
