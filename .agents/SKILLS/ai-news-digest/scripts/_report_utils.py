...
WORK_REPORTS_DIR = PROJECT_DIR / ".reports"
...
def sync_work_reports(reports_dir: Path | None = None) -> None:
    """Mirror committed reports/ into .reports/ for legacy scripts."""
    root = reports_dir or REPORTS_DIR
    WORK_REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    for path in root.iterdir():
        if path.is_file() and path.suffix in {".html", ".json"}:
            target = WORK_REPORTS_DIR / path.name
