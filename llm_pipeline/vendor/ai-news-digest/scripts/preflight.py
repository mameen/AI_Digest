...
What this script fetches (all in parallel, ~15s):
  ...
  ✓ YouTube      — IBM, Google Cloud, The Stack, Nate Herk, NetworkChuck, AZisk
...
from fetch_video_chapters   import (
    ...
    fetch_video_info,
    to_story_cards as chapters_to_stories,
    make_category,
)
from youtube_channels import fetch_youtube_secondary_category
...
def _fetch_aisearch_category() -> dict:
    ...
    cat = attach_video_metadata(make_category("aisearch", stories), info)
    return cat


def _fetch_youtube_category() -> dict:
    return fetch_youtube_secondary_category()
    ...
    for key, fn in section_fns.items():
        ...
        if not force:
            ...
            if cached is not None:
                ...
                if key == "youtube" and isinstance(cached, dict) and cached.get("sources"):
                    n = f"{n} topics / {len(cached.get('sources') or [])} channels"
