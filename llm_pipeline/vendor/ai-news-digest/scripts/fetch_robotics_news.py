...
def _item_body_html(item: ET.Element) -> str:
    """RSS ``description`` or ``content:encoded`` body (often HTML)."""
...
def parse_feed(xml_bytes: bytes, source: str, limit: int = 10) -> list[dict]:
    """Parse an RSS or Atom feed into ``{source, title, url, body_html}`` article dicts."""
...
def to_story_cards(raw_results: dict[str, list[dict]]) -> list[dict]:
    ...
    from link_extract import attach_story_embedded_links, html_embedded_text
    ...
    for articles in raw_results.values():
        for a in articles:
            if "error" in a or a["url"] in seen_urls:
                continue
            seen_urls.add(a["url"])
            body = html_embedded_text(a.get("body_html") or "")
            story = make_story(
                a["title"],
                a["url"],
                a["source"],
                raw_snippet=body[:2000],
                id_prefix="robo",
                extra_tags=["robotics", "embodied-ai"],
            )
