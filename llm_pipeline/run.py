...
Usage:
    ...
    python run.py --server                     # local web UI + admin API (127.0.0.1:8765)

Outputs land under ``llm_pipeline/reports/``, ``diagnostics/``, and ``.cache/``.
...
def main() -> None:
    ...
    parser.add_argument(
        "--server",
        action="store_true",
        help="Serve local web UI (reports, diagnostics, admin) + control API; no pipeline run",
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not open a browser when using --server",
    )
